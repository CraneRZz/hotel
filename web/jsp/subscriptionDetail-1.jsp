...
<div id="container">
 ...
 <jsp:include page="top.jsp" flush="true"/>
  ...
  <div id="content" style="border:solid 1px #ffffff;width:90%;margin:auto;padding-top:10px">
    <form action="${pageContext.request.contextPath }/subscription/updateDetail" method="post">
        ...
        <div class="row">
          <div class="column1">
            ...
            <div class="column_2">
              <span>${membersubscription.no}</span>
            </div>
          </div>
          ...
          <div class="column2">
            ...
            <div class="column_2">
              <span><fmt:formatDate pattern="yyyy-MM-dd HH:mm" value="${membersubscription.cretime}"/></span>
            </div>
          </div>
          ...
          <div class="column3">
            ...
            <div class="column_2">
              <span>${membersubscription.linkman}</span>
            </div>            
          </div>
          ...
          <div class="column4">
            ...
            <div class="column_2">
              <!--  <select name="status" id="status">
                <option value="1" <c:if test="${membersubscription.status==1}">selected</c:if>>待确认</option>
                <option value="2" <c:if test="${membersubscription.status==2}">selected</c:if>>已确认</option>
                <option value="0" <c:if test="${membersubscription.status==0}">selected</c:if>>已取消</option>
              </select>-->
               <c:if test="${membersubscription.status==1}">待确认</c:if>
                <c:if test="${membersubscription.status==2}">已确认</c:if>
                <c:if test="${membersubscription.status==0}">已取消</c:if>
            </div>
          </div>
        </div>
        <div class="row">
          ...
          <div class="column2">
            ...
            <div class="column_2">
              <span>${membersubscription.phone}</span>
            </div>
          </div>
          ...
          <div class="column3">
            ...
            <div class="column_2">
              <span>${membersubscription.email}</span>
            </div>            
          </div>
          <div class="column4">
            <span>&nbsp;</span>
      ...
      <tr>
		...
		<c:forEach items="${memberSubscriptionDetail}" var="subdetail">
    ...
    <div class="submit">
      <input type="button" value="返回" onclick="location='${pageContext.request.contextPath }/subscription/historySubscription'"/>
    </div>
  </div>
  <hr />
  <div id="footer">
